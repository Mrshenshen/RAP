...
class RayPPOTrainer:
    ...
    def _create_dataloader(self) -> None:
        # print("self.prompt")
        # print(self.prompt)
        # #self.config.data.format_prompt
        # print("self.config.data.format_prompt")
        # print(self.config.data.format_prompt)
        # exit()
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            test_batch.meta_info = self.config.worker.rollout.val_override_config
            test_batch.non_tensor_batch["uid"] = np.array(
            [str(uuid.uuid4()) for _ in range(len(test_batch.batch))], dtype=object
            )
            repeat_n = self.config.worker.rollout.n 
            test_batch = test_batch.repeat(repeat_times=repeat_n, interleave=True)
            test_batch, pad_size = pad_dataproto_to_divisor(test_batch, self.actor_rollout_wg.world_size)
            test_output_gen_batch = self.actor_rollout_wg.generate_sequences(test_gen_batch)
            test_output_gen_batch = unpad_dataproto(test_output_gen_batch, pad_size=pad_size)
            output_ids = test_output_gen_batch.batch["responses"]
            output_texts = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in output_ids]
            grouped_outputs = [output_texts[i*repeat_n : (i+1)*repeat_n] for i in range(len(output_texts)//repeat_n)]
            grouped_labels = test_batch.non_tensor_batch["ground_truth"].tolist() 
            sample_outputs.extend(grouped_outputs)
            sample_labels.extend(grouped_labels)
            test_batch = test_batch.union(test_output_gen_batch)
            # evaluate using reward_function
            reward_tensor, reward_metrics = self.val_reward_fn(test_batch)
            for idx, sample in enumerate(test_batch):
                prompt = sample.non_tensor_batch['problem']
                response = sample.non_tensor_batch['ground_truth']
                prompt = self.tokenizer.batch_decode(test_output_gen_batch[idx].batch['prompts'], skip_special_tokens=True)
                prompt = ''.join([item for item in prompt if item])
                response = self.tokenizer.batch_decode(test_output_gen_batch[idx].batch['responses'], skip_special_tokens=True)
                response = ''.join([item for item in response if item])
                reward = reward_metrics['overall'][idx]
                sample_data.append({
                    "onlyid": test_batch[idx].non_tensor_batch['onlyid'],
                    "id": test_batch[idx].non_tensor_batch['id'],
                    "prompt": prompt,
                    "answer": response,
                    "question": test_batch[idx].non_tensor_batch['problem'],
                    "ground_truth": test_batch[idx].non_tensor_batch['ground_truth'],
                    "reward": reward, 
        ...
        save_dir = os.path.join(f'/RAP/output/result')
        os.makedirs(save_dir, exist_ok=True)
        with open(os.path.join(save_dir, f"result.json"), 'w', encoding='utf-8') as f:
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        for idx, sample in enumerate(batch):
                            ...
                            sample_data.append({
                                ...
                                "prompt": prompt,
                                "answer": response,
                                "question": batch[idx].non_tensor_batch['problem'],
                                "ground_truth": batch[idx].non_tensor_batch['ground_truth'],
                                "reward": reward, 
                                "response_length": len(response)
                            })   
                        save_dir = os.path.join('/RAP/output/samples', "train")
                        save_dir_reward_tensor = os.path.join('/RAP/output/reward', "tensor")
                        save_dir_reward_metrics = os.path.join('/RAP/output/reward', "metrics")
                        os.makedirs(save_dir, exist_ok=True)
                        os.makedirs(save_dir_reward_tensor, exist_ok=True)
                        os.makedirs(save_dir_reward_metrics, exist_ok=True)
                        ...
                        np.save(os.path.join(save_dir_reward_tensor, f"step_{self.global_step}_reward_tensor.npy"), reward_tensor_numpy)
                        with open(os.path.join(save_dir_reward_metrics, f"step_{self.global_step}_reward_metrics.json"), "w") as f:
